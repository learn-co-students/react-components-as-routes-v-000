...
The React Router API comes with two options to add Links: __<Link>__ and __<NavLink>__. The both have the same base level functionality that will update the browser url and render the __Route__ component, but __<NavLink>__ comes with some additional features that are great for navbars like: 
